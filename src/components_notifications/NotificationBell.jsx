...
const NotificationBell = () => {
  ...
  const handleWebSocket = () => {
    ...
    const wsUrl = `ws://localhost:8080/ws?ID_usuario=${uid}`; // Enviar `uid` como query parameter
    ...
    ws.current.onopen = () => {
      ...
      retryCount.current = 0; // Reinicia el contador de reconexión
    };
    ...
    ws.current.onclose = (event) => {
      ...
      if (retryCount.current < maxRetries) {
        console.log(`Intentando reconectar... (${retryCount.current + 1})`);
        retryCount.current += 1;
        setTimeout(handleWebSocket, 3000); // Esperar 3 segundos antes de intentar reconectar
      } else {
  ...
  const handleDeleteNotification = (id) => {
    setNotifications((prevNotifications) =>
      prevNotifications.filter((notification) => notification.ID_remitente !== id)
    );
  };
  ...
  return (
    <div className="relative">
      ...
      >
        <BellIcon
          className={`h-6 w-6 ${
            showNotifications ? "text-[#FFD166]" : "text-gray-200"
          }`}
        />
        ...
        <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-300 rounded-lg shadow-lg overflow-y-auto max-h-64">
          <ul>
            {notifications.map((notification) => (
              <li
                key={notification.ID_remitente}
                className="p-2 border-b border-gray-200 text-black flex justify-between items-center"
              >
