...
const PracticasList = () => {
  ...
  const handleSearch = (e) => {
    ...
    if (searchTerm.trim() !== "") {
      setSearchHistory((prevHistory) => [
        ...new Set([searchTerm, ...prevHistory]),
      ]);
      fetchPracticas();
    }
  };
  ...
  const filteredPracticas = practicas.filter((practica) => {
    ...
    const matchesFilters = Object.entries(filters).every(([key, value]) => {
      ...
      const practicaValue =
        practica[key] || practica[key.charAt(0).toUpperCase() + key.slice(1)];
      if (key === "area_practica" || key === "ubicacion") {
        return (
          practicaValue && practicaValue.toLowerCase() === value.toLowerCase()
        );
        ...
        return (
          practicaValue &&
          practicaValue.toLowerCase().includes(value.toLowerCase())
        );
      }
    });
  ...
  return (
    <div
      className={`container mx-auto p-4 ${
        currentTheme === "dark" ? "bg-gray-900" : "bg-[#DAEDF2]"
      }`}
    >
      <h1
        className={`text-2xl font-bold mb-4 ${
          currentTheme === "dark" ? "text-white" : "text-[#1D4157]"
        }`}
      >
        ...
        <div className="lg:w-1/3 mb-4 lg:mb-0 mr-4 w-full">
          ...
          >
            <form
              ...
              className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 mb-4 w-full"
            >
              <input
                type="text"
                value={searchTerm}
          ...
          {filteredPracticas.length > 0 ? (
            filteredPracticas.map((practica) => (
              ...
              >
                ...
                >
                  {practica.Area_practica && (
                    <p>Área: {practica.Area_practica}</p>
                  )}
      ...
      {showPostulacion && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div
            ...
            } p-4 rounded-lg max-w-md w-full`}
          >
            <PostulacionPractica
